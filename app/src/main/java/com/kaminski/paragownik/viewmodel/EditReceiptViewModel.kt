...
class EditReceiptViewModel(application: Application) : AndroidViewModel(application) {
                ...
                storeDao.getStoreByIdFlow(receiptWithClient.receipt.storeId)
                    .map { store ->
                        Pair(
                            receiptWithClient,
                            store?.storeNumber
                        ) // Zwróć parę: ReceiptWithClient i storeNumber
                    }
            }
    ...
    suspend fun updateReceiptAndClient( // Poprawiona funkcja updateReceiptAndClient
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val existingReceipt =
                    receiptDao.getReceiptById(receiptId) // Użyj getReceiptById do pobrania paragonu po ID

                if (existingReceipt == null) {
                    Log.e(
                        "EditReceiptViewModel",
                        "Nie znaleziono paragonu o ID: $receiptId"
                    ) // Zmieniono komunikat logu
                ...
                if (storeNumberString != null) {
                    ...
                    if (store == null) {
                        ...
                        store =
                            storeDao.getStoreByNumber(storeNumberString) // Pobierz nowo dodaną drogerię
                        Log.d(
                            "EditReceiptViewModel",
                            "Dodano nową drogerię o numerze: ${storeNumberString}"
                        )
                    }
                ...
                // Aktualizuj paragon
                val updatedReceipt =
                    existingReceipt.copy( // Użyj copy, aby zaktualizować istniejący paragon
                ...
                existingClient?.let { client -> // Upewnij się, że klient istnieje
                    val updatedClient =
                        client.copy(description = clientDescription) // Użyj copy, aby zaktualizować klienta
                    clientDao.updateClient(updatedClient) // Użyj updateClient DAO
                    Log.d(
                        "EditReceiptViewModel",
                        "Zaktualizowano klienta. ID Klienta: ${updatedClient.id}, Opis Klienta: ${updatedClient.description}"
                    )
                } ?: run {
                ...
                Log.d(
                    "EditReceiptViewModel",
                    "Paragon i Klient zaktualizowane pomyślnie. ID Paragonu: $receiptId, Numer Drogerii: $storeNumberString, Numer Paragonu: $receiptNumber, Data Paragonu: $receiptDateString, Data Weryfikacji: $verificationDateString, Opis Klienta: $clientDescription"
                )
    ...
    suspend fun deleteReceipt(receipt: Receipt): Boolean =
        suspendCancellableCoroutine { continuation -> // Poprawiona funkcja deleteReceipt
            viewModelScope.launch(Dispatchers.IO) {
                try {
                    val receiptToDelete =
                        receiptDao.getReceiptById(receipt.id) // Pobierz paragon do usunięcia (dla pewności)
                    ...
                    receiptToDelete?.let {
                        ...
                        if (clientId != null) {
                            val clientReceiptsCount =
                                receiptDao.getReceiptsForClientCount(clientId) // Dodaj funkcję DAO
                            if (clientReceiptsCount == 0) {
                                ...
                                clientToDelete?.let { client ->
                                    ...
                                    Log.d(
                                        "EditReceiptViewModel",
                                        "Klient usunięty automatycznie (brak paragonów). ID Klienta: ${client.id}"
                                    )
                                }
                            }
                        }
                        ...
                        val storeReceiptsCount =
                            receiptDao.getReceiptsForStoreCount(storeId) // Dodaj funkcję DAO
                        if (storeReceiptsCount == 0) {
                            ...
                            storeToDelete?.let { store ->
                                ...
                                Log.d(
                                    "EditReceiptViewModel",
                                    "Drogeria usunięta automatycznie (brak paragonów). ID Drogerii: ${store.id}"
                                )
                            }
                        }
                        ...
                        Log.d(
                            "EditReceiptViewModel",
                            "Paragon usunięty pomyślnie. ID Paragonu: ${receipt.id}"
                        )
                        continuation.resume(true)
                    } ?: {
                        Log.e(
                            "EditReceiptViewModel",
                            "Nie znaleziono paragonu do usunięcia. ID Paragonu: ${receipt.id}"
                        )
                        continuation.resume(false)
                    }
    ...
    suspend fun deleteClient(client: Client): Boolean =
        suspendCancellableCoroutine { continuation -> // Poprawiona funkcja deleteClient
            viewModelScope.launch(
                Dispatchers.IO,
                CoroutineStart.UNDISPATCHED
            ) { // ZMIANA - CoroutineStart.UNDISPATCHED
                Log.d(
                    "EditReceiptViewModel",
                    "deleteClient: Rozpoczęto usuwanie klienta. ID Klienta: ${client.id}"
                )
                try {
                    val clientToDelete =
                        clientDao.getClientById(client.id) // Pobierz klienta do usunięcia (dla pewności)
                    clientToDelete?.let {
                        Log.d(
                            "EditReceiptViewModel",
                            "deleteClient: Klient pobrany z bazy danych. ID Klienta: ${client.id}"
                        )
                        ...
                        storeIdToDeleteCheck =
                            receiptDao.getReceiptWithClient(receiptIdForDeletion)?.receipt?.storeId


                        Log.d(
                            "EditReceiptViewModel",
                            "deleteClient: storeId paragonu pobrane. storeId: $storeIdToDeleteCheck"
                        )


                        Log.d(
                            "EditReceiptViewModel",
                            "deleteClient: Usuwanie klienta z bazy danych. ID Klienta: ${client.id}"
                        )
                        clientDao.deleteClient(it) // Usuń klienta - kaskada powinna usunąć paragony
                        Log.d(
                            "EditReceiptViewModel",
                            "deleteClient: Klient usunięty z bazy danych. ID Klienta: ${client.id}"
                        )


                        Log.d(
                            "EditReceiptViewModel",
                            "deleteClient: Sprawdzanie drogerii do automatycznego usunięcia."
                        )
                        ...
                        storeIdToDeleteCheck?.let { storeId ->
                            ...
                            Log.d(
                                "EditReceiptViewModel",
                                "deleteClient: Liczba paragonów drogerii po usunięciu klienta: $storeReceiptsCount, ID Drogerii: $storeId"
                            )
                            if (storeReceiptsCount == 0) {
                                // Usuń drogerię, jeśli nie ma już paragonów
                                val storeToDelete = storeDao.getStoreById(storeId)
                                storeToDelete?.let { store ->
                                    Log.d(
                                        "EditReceiptViewModel",
                                        "deleteClient: Usuwanie drogerii z bazy danych. ID Drogerii: ${store.id}"
                                    )
                                    storeDao.deleteStore(store)
                                    Log.d(
                                        "EditReceiptViewModel",
                                        "deleteClient: Drogeria usunięta z bazy danych. ID Drogerii: ${store.id}"
                                    )
                                }
                            }
                        }
                        ...
                        Log.d(
                            "EditReceiptViewModel",
                            "deleteClient: Usuwanie klienta zakończone sukcesem. ID Klienta: ${client.id}"
                        )
                        continuation.resume(true)
                        ...
                        Log.e(
                            "EditReceiptViewModel",
                            "deleteClient: Nie znaleziono klienta do usunięcia. ID Klienta: ${client.id}"
                        )
                        continuation.resume(false)
                    }
                    ...
                    Log.e(
                        "EditReceiptViewModel",
                        "deleteClient: Błąd podczas usuwania klienta i paragonów.",
                        e
                    )
                    continuation.resume(false)
                }
